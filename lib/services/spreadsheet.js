...
class GSheet2Sins2 {
    ...
    insertLocalizedTextFromDirectory(data) {
        ...
        this.writeEntity(this.gameFolder.localized_text, 'en.localized_text', obj)
    }

    insertEntityDataFromDirectory(entities, data) {
        const entityDir = fs.readdirSync(this.gameFolder.entities).filter((e) => e.endsWith('.unit'))
        entityDir.forEach((file) => {
            ...
            this.writeEntity(this.gameFolder.entities, file, this.entityContents)
        })
    }
        ...
        for (const entity of docEntities) {
            ...
            if (entity.Filename.startsWith('<race>')) {
                for (const race of ['amarr', 'gallente', 'minmatar', 'caldari']) {
                    ...
                    this.writeEntity(this.gameFolder.entities, entityName + '.unit', {})
                }
                continue
            }
            this.writeEntity(this.gameFolder.entities, entity.Filename + '.unit', {})
        }
