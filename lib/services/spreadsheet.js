...
const Sins2Data = require('./sins2-data')

class GSheet2Sins2 {
    constructor({ modFolder: modFolder, sheetId: sheetId, apiKey: apiKey }) {
        this.gameFolder = ['entities', 'localized_text'].reduce((obj, curr) => {
            obj[curr] = path.resolve(modFolder, curr)
            return obj
        }, {})
    ...
    entityExists(entity) {
        return fs.existsSync(path.resolve(this.gameFolder.entityFolder, entity))
    }

    readEntity(entity) {
        const contents = fs.readFileSync(path.resolve(this.gameFolder.entityFolder, entity), 'utf-8')
    ...
    insertLocalizedTextFromDirectory(data) {
        ...
        this.writeEntity(this.gameFolder.localizedTextFolder, 'en.localized_text', obj)
    }

    insertEntityDataFromDirectory(entities, data) {
        const entityDir = fs.readdirSync(this.gameFolder.entityFolder).filter((e) => e.endsWith('.unit'))
        entityDir.forEach((file) => {
            ...
            this.writeEntity(this.gameFolder.entityFolder, file, this.entityContents)
        })
    }
        ...
        for (const entity of docEntities) {
            ...
            if (entity.Filename.startsWith('<race>')) {
                for (const race of ['amarr', 'gallente', 'minmatar', 'caldari']) {
                    ...
                    this.writeEntity(this.gameFolder.entityFolder, entityName + '.unit', {})
                }
                continue
            }
            this.writeEntity(this.gameFolder.entityFolder, entity.Filename + '.unit', {})
        }
...
module.exports = async function main(sheetId, modDirectory, apiKey) {
    ...
    const unitData = [
        ...
        'physics.time_to_max_linear_speed',
        'physics.time_to_max_angular_speed',
        'physics.linear_acceleration_angle',
        'physics.max_bank_angle',
    ]

    const [shipBasic, shipNavigation, shipTank, shipLocal] = await Promise.all([
        await GSheet.fetchDataFromSheet(queryInterface.basic),
        await GSheet.fetchDataFromSheet(queryInterface.navigation),
        await GSheet.fetchDataFromSheet(queryInterface.tank),
        await GSheet.fetchDataFromSheet(queryInterface.localisation),
    ])
